  ...
  for (i=0; i<p_ctable->attr_length; i++)
  {
    ...
    if (p_ctable->p_attr[i].update & _UPDATE_INIT)
    {
      if ((addr+length) < p_ctable->mem_length)
      {
        ...
        {
          if (((p_ctable->p_attr[i].attr & _ATTR_EEPROM) > 0 && eepromReadByte(8) != 1) || (p_ctable->p_attr[i].attr & _ATTR_EEPROM) == 0)
          {
            initialized = true;
    ...
    if (p_ctable->p_attr[i].update & _UPDATE_RD)
    {
      ...
      {
        if (p_ctable->p_mem != NULL && initialized == false)
        {
...
uint8_t ctableRead(ctable_t *p_ctable, uint16_t addr, uint8_t *p_data, uint16_t length)
{
  ...
  for (i=0; i<p_ctable->attr_length; i++)
  {
    ...
    if (node_addr_start >= addr_start && node_addr_end <= addr_end)       // �뀒�씠釉붿쓽 二쇱냼 踰붿쐞媛� �씫湲� 二쇱냼 踰붿쐞�뿉 �룷�븿�릺�뒗 寃쎌슦
    {
      update = true;
    }
    if (addr_start >= node_addr_start && addr_start < node_addr_end)      // �씫湲� 二쇱냼�쓽 �떆�옉 二쇱냼媛� �뀒�씠釉� 二쇱냼�쓽 踰붿쐞�뿉 �엳�뒗 寃쎌슦
    {
      update = true;
      node_addr_start = addr_start;
    }
    if (addr_end > node_addr_start && addr_end <= node_addr_end)          // �씫湲� 二쇱냼�쓽 留덉�留� 二쇱냼媛� �뀒�씠釉� 二쇱냼�쓽 踰붿쐞�뿉 �엳�뒗 寃쎌슦
    {
      update = true;
...
uint8_t ctableWrite(ctable_t *p_ctable, uint16_t addr, uint8_t *p_data, uint16_t length)
{
  ...
  for (i=0; i<p_ctable->attr_length; i++)
  {
    ...
    // �벐湲곗쓽 寃쎌슦�뒗 �떆�옉 二쇱냼�� �궗�씠利덇� 媛숈븘�빞 �븳�떎.
      ...
      if (p_ctable->p_attr[i].attr & _ATTR_WR)
      {
        ...
        if (p_ctable->p_attr[i].attr & _ATTR_EEPROM)
        {
          for (j=node_addr_start; j<node_addr_end; j++)
          {
            ...
            {
              #ifdef _USE_HW_EEPROM
              if (p_ctable->p_mem[j] != eepromReadByte(j))
              {
